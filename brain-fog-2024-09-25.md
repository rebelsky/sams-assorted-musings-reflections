...
Unfortunately, writing unit tests remains a non-trivial task. At larger institutions, faculty probably dump the task on their teaching assistants. My mentors don't really have the time to do so. I'll admit that I also don't necessarily trust others to write tests. I am, after all, a control freak.

I've already spent more than a day writing unit tests for our latest mini-project. Today, I'm on tests for the final method, `eqv`. Students are building structures (somewhat like graphical UIs, but with ASCII). We are considering three types of equality: "two things look the same" (`equal`), "two things were built in essentially the same way" (`eqv`), and "two things occupy the same memory location (`eq`). I've provided students with `equal` and `eq` and have asked them to write `eqv`, which is the hardest of the three.

To test their code, I'm making a lot of objects that all look the same, but are constructed differently. Sometimes the construction is very similar. Sometimes it's very different.

Here's a method I'd written that compares "grids" that are build in different ways. _Warning! Horrid code follows!_
...
I'm an experienced enough programmer that I should have started with something better. However, it appears that I'm in a bit of a brain freeze. So I wrote bad code to start with. But I eventually realized I needed to clean it up, and did so before I wrote too much bad code.

What are the fixes? First, when there are bunch of related things, you should put them in an array. I put all the `exesGrid#a` objects in one array (`exesGridsA`) and all the `exesGrid#b` objects in another (`exesGridsB`). I did something similar for the other types.

Second, you should then write for loops to deal with the pairs of objects. Here are the two methods I wrote. (Hopefully, I got them right.) The first compares different objects of the same type.
...
There are also other benefits to this approach. Most importantly, if I decided to add an eighth kind of horizontal composition, I would only need to make one or two changes in the program: adding an element to `exesHCompsA` (as well as an `exesHCompsB` that I haven't mentioned yet). No need to search my code for other lines to add! ("Let's see ... everywhere I mention `exesHComp6a`, I should add a similar assertion for `exesHZComp7a` ....") Making additional examples  easy to add encourages more testing (or at least doesn't discourage more testing), and more testing is generallly good (within limits).

In addition, if we suddenly added a new type to our project, it wouldn't take me long to add tests for that type. Otherwise, I'd been doing a lot of copying and pasting and changing.

The new approach also cuts the number of lines in the program. That makes the program more readable. And it reminds us that "lines per hour" is a bad way to measure programmer productivity; sometimes the best code (or even just decent code) cuts the number of lines, rather than increasing them.

There is, however, a significant difficulty in implementing this approach. When we've written separate assertions for each pairing it's easy to tell which pair of values failed the test. If we weren't carefully, we'd just see a report that we failed on one of the pairs, but be unable to tell which one. That would be sad. As you may be able to tell, I spent a bit of extra effort ensuring that the error message in the assertion makes it clear which pair we have to consider.
