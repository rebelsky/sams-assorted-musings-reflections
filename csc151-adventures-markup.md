...
[LaTeX](https://www.latex-project.org/) is powerful and
it's a markup language [2] that most CS faculty know well.
...
standards of the broader Racket community.  Scribble had also been on my
list of languages to learn, particularly because at least some digital
humanities Racket work uses Scribble.  Perhaps most importantly,
Scribble provides a really nice interface for showing what happens
in the interactions pane.  For example, if I type
...
Scribble is also a fairly niche language, so help is limited.  It's also
not supported by [Pandoc](https://pandoc.org/), which means that I have
more effort in translating formats.  Because it's a niche language and
I don't completely understand it, it's hard to achieve all of the things
I want.  For example, while I appreciate the ability to include image-making
...
to work with Scribble than, say, Markdown.
...
that I would miss the capability to quickly generate examples.  I'd
written something to something similar a decade or so ago, but I haven't
used it for at least five years, and I'm not sure that it works any more.
...
At that point, I thought the process would be fairly straightforward.
...
I would need to manually update the alt text for each image.  But at least
...
Lovely, isn't it?  After some reflection, I realized
...
turnout that writing the plugin was easier than I expected.
